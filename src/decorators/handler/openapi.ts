...
export function BindOperation(operationId: string) {
  return function (target: any, methodName: string) {
    ...
    if (existing?.path) {
      throw new Error(
        `Controller ${target.constructor.name} method ${methodName} cannot both be bound to an operation and have http methods specified.`
      );
    }

    if (existing?.openapiFragment) {
      throw new Error(
        `Controller ${target.constructor.name} method ${methodName} cannot both be bound to an operation and have openapi fragments specified.  This may happen if you have used @PathParam or @QueryParam instead of @BoundParam`
      );
    }
