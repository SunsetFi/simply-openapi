...
describe("E2E: Cookie Param", function () {
  ...
  class WidgetController {
    ...
    getOptionalParam(
      @CookieParam("foo", "integer", { description: "The foo parameter" })
      foo: number,
    ) {
      handler(foo);
      return { bar: true };
    ...
    getRequiredParam(
      @RequiredCookieParam("foo", "integer", {
      ...
      setTimeout(() => {
        try {
          expectNextCalledWithError(
            next,
            BadRequest,
            /Cookie parameter "foo" is invalid: value must be integer/,
          );
